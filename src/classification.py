...
def parse_args():
    ...
    parser.add_argument("--provider", type=str, help="Provider for workflow classification")
    parser.add_argument("--api_key", type=str, help="API key for workflow classification")
    parser.add_argument("--api_base", type=str, help="API base for workflow classification")
    parser.add_argument("--max_tokens", type=int, help="Maximum tokens for workflow classification")
    parser.add_argument("--temperature", type=float, help="Temperature for workflow classification")
    ...
    parser.add_argument("--wait_time", type=float, help="Wait time between classifications to avoid rate limiting")
...
class WorkflowClassifier:
    ...
    def __init__(
        ...
        if self.provider == "openai":
            ...
            if can_use_cuda:
                print(f"CUDA is available with {torch.cuda.device_count()} visible devices")
                cuda_visible_devices = os.environ.get("CUDA_VISIBLE_DEVICES", "")
                if cuda_visible_devices:
                    print(f"Using GPUs specified by CUDA_VISIBLE_DEVICES: {cuda_visible_devices}")
                
                if gpu_ids and "," in gpu_ids:
                    print(f"Multi-GPU: Model will be loaded with device_map='auto'. Pipeline will use model's device map.")
                    model_load_kwargs["device_map"] = "auto"
                elif gpu_ids:
                    print(f"Single-GPU: Pipeline will target device 0 (relative to visible devices).")
                    pipeline_init_kwargs["device"] = 0 
                else:
                    print(f"No specific GPUs (user) & CUDA available: Model will be loaded with device_map='auto'. Pipeline will use model's device map.")
                    model_load_kwargs["device_map"] = "auto"
            else:
                # No CUDA available
    ...
    def classify(
        ...
        prompt = PROMPT.format(
            classes=', '.join(possible_classes),
            sequence=events_description
        )
    ...
    def classify_batch(
        ...
        if self.provider != "huggingface":
            raise ValueError("Batch processing is only supported for HuggingFace provider.")
...
def create_heatmap(true_labels, pred_labels, figsize=(10, 8), output_path=None):
    ...
    report = pd.DataFrame(classification_report(true_labels, pred_labels, labels=label_list, output_dict=True, zero_division=0))
    ...
    plt.title('Classification Performance Metrics', fontsize=16, fontweight='bold', pad=20)
...
def create_confusion_matrix(true_labels, pred_labels, figsize=(10, 8), output_path=None):
    """
...
def evaluation(processor, args):
    ...
    if args.gpus:
        logger.info(f"Evaluation using GPUs: {args.gpus} (set via CUDA_VISIBLE_DEVICES)")
    else:
    ...
    if args.provider.lower() == "huggingface" and args.batch_size > 1:
        # Batch processing for HuggingFace provider
        logger.info(f"Using batch processing (batch_size={args.batch_size}) for HuggingFace provider")
        ...
        # Process batches
        for i in tqdm.tqdm(range(0, len(batch_descriptions), args.batch_size), desc="Processing batches"):
            batch_desc = batch_descriptions[i:i+args.batch_size]
            batch_pred_classes = classifier.classify_batch(
                event_descriptions=batch_desc,
                possible_classes=list(class_names)
            )
            
            # Process predictions
            for j, pred_class in enumerate(batch_pred_classes):
                if pred_class not in class_names:
                    logger.info(f"Predicted class {pred_class} not found in class names {class_names}")
        ...
        for i, item in tqdm.tqdm(enumerate(classification_data), total=len(classification_data)):
            # Create description for the workflow
            description = "\n".join(item["event_descriptions"])
    ...
    report = create_heatmap(all_labels, all_preds, output_path=os.path.join(output_dir, "heatmap.png"))
    ...
    cm = create_confusion_matrix(all_labels, all_preds, output_path=os.path.join(output_dir, "confusion_matrix.png"))
