...
def load_config(config_path: str) -> Dict[str, Any]:
    ...
    with open(config_path, "r") as f:
        if config_path.endswith(".yaml") or config_path.endswith(".yml"):
            ...
            raise ValueError(
                f"Unsupported configuration file format: {config_path} (must be .yaml or .yml)"
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--provider", type=str, help="Provider for workflow segmentation"
    )
    parser.add_argument("--api_key", type=str, help="API key for workflow segmentation")
    parser.add_argument(
        "--api_base", type=str, help="API base for workflow segmentation"
    )
    parser.add_argument(
        "--max_tokens", type=int, help="Maximum tokens for workflow segmentation"
    )
    parser.add_argument(
        "--temperature", type=float, help="Temperature for workflow segmentation"
    )
    ...
    parser.add_argument(
        "--wait_time",
        type=float,
        help="Wait time between segmentations to avoid rate limiting",
    )

    # Segmentation specific arguments
    parser.add_argument(
        "--min_workflows", type=int, help="Minimum number of workflows to concatenate"
    )
    parser.add_argument(
        "--max_workflows", type=int, help="Maximum number of workflows to concatenate"
    )
    parser.add_argument("--num_samples", type=int, help="Number of samples to create")
    parser.add_argument(
        "--tolerance", type=int, help="Tolerance for boundary detection"
    )
...
class WorkflowSegmenter:
    ...
    def segment(
        self, events_description: List[str], process_definitions_text: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        sequence_text = "\n".join(
            [f"{i}. {event}" for i, event in enumerate(events_description)]
        )

        prompt = SEGMENTATION_PROMPT.format(
            process_definitions=process_definitions_text, sequence=sequence_text
        )
        ...
        if self.provider == "openai":
            ...
            try:
                ...
                logger.warning(
                    "Could not properly extract Hugging Face response content"
                )
...
def calculate_segmentation_edit_distance(
    true_boundaries: List[int], pred_boundaries: List[int]
) -> int:
    """
...
def visualize_segmentation(
    ...
    for i, seg in enumerate(true_segments):
        ...
        ax1.text(
            start + width / 2, 0, f"Segment {i+1}", ha="center", va="center", fontsize=9
        )
...
def evaluation(processor, args):
    ...
    output_dir = os.path.join(
        args.output_dir, "segmentation", args.model_name.split("/")[-1]
    )
    ...
    try:
        ...
        for process in process_defs:
            if process["name"].lower() in [p.lower() for p in processor.process_names]:
                process_definitions_text += (
                    f"- {process['name']}: {process['description']}\n"
                )
    ...
    logger.info(
        f"Running segmentation evaluation with {args.model_name} on {len(segmentation_data)} sequences..."
    )
    ...
    for sequence in tqdm.tqdm(segmentation_data, desc="Evaluating segmentation"):
        ...
        for i in range(len(sequence["segment_boundaries"]) - 1):
            ...
            true_segments.append({"start_index": start_idx, "end_index": end_idx})
        ...
        pred_boundaries = [
            seg["start_index"] for seg in pred_segments if seg["start_index"] > 0
        ]

        # Evaluate boundary detection
        boundary_metrics = evaluate_boundary_detection(
            true_boundaries=sequence["segment_boundaries"][
                1:-1
            ],  # Skip the first and last boundary (0) and (last)
        ...
        visualize_segmentation(
            ...
            output_path=os.path.join(
                output_dir, f"visualization_{sequence['sequence_id']}.png"
            ),
        )
    ...
    # Log results
    logger.info(
        f"Segmentation Boundary Detection - Precision: {avg_precision:.4f}, Recall: {avg_recall:.4f}, F1: {avg_f1:.4f}"
    )
...
def create_summary_plots(results, output_dir):
    ...
    plt.savefig(
        os.path.join(output_dir, "performance_vs_workflows.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    for r in results:
        bin_idx = (
            min(num_bins - 1, int((r["sequence_length"] - min_len) / bin_width))
            if bin_width > 0
            else 0
        )
    ...
    y_f1 = [
        np.mean([r["boundary_f1"] for r in length_bins[label]]) for label in x_labels
    ]
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "performance_vs_length.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
